...
def fit_capm(
    ...
    market_returns:
        [T] 또는 [B, T]
    risk_free_rates:
        scalar 또는 market_returns에 broadcast 가능한 Tensor

    반환 shape:
        alpha, beta: [N] 또는 [B, N]
        residuals, fitted_returns: [T, N] 또는 [B, T, N]
    """
    if eps <= 0:
    ...
    if fit_intercept:
        ...
        beta = (
            market_centered.unsqueeze(-1) * asset_centered
        ...
        beta = (
            market_excess.unsqueeze(-1) * asset_excess
        ).sum(dim=1) / denominator.unsqueeze(-1)
