...
def build_markowitz_layer(
    ...
    """
    Constrained mean-variance optimization layer.
...
def covariance_to_risk_factor(
    ...
    """
    covariance를 MVO layer의 risk factor로 변환한다.

    반환값 L은 다음을 만족한다.
        L^T L = risk_aversion * covariance
    ...
    try:
        ...
        minimum_eigenvalue = torch.linalg.eigvalsh(
            symmetric_covariance.detach()
        ).min().item()
        raise RuntimeError(
            "Covariance의 Cholesky 분해에 실패했습니다. "
            "Dataset covariance_jitter, effective covariance 또는 "
            "PSD 처리를 확인하세요. "
