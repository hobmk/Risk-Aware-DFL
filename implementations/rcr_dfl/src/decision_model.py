...
from .effective_covariance import build_effective_covariance
...
class RCRMLPWithMarkowitz(nn.Module):
    """4-layer MLP와 residual-correlation-aware MVO layer를 연결한다."""
    ...
    def __init__(
        ...
        self.markowitz_layer = build_markowitz_layer(n_assets=n_assets, max_weight=max_weight)
    ...
    def forward(
        ...
        covariance_solver = covariance.to(device="cpu", dtype=torch.float64)
        a_res_solver = a_res.to(device="cpu", dtype=torch.float64)
        effective_covariance = build_effective_covariance(
            covariance=covariance_solver,
            residual_risk_matrix=a_res_solver,
            eta=self.eta,
        ...
        predicted_returns_solver = predicted_returns.to(device="cpu", dtype=torch.float64)
        predicted_weights, = self.markowitz_layer(predicted_returns_solver, risk_factor)
    ...
    def solve_oracle(self, true_returns: torch.Tensor, risk_factor: torch.Tensor) -> torch.Tensor:
        true_returns_solver = true_returns.to(device="cpu", dtype=torch.float64)
        oracle_weights, = self.markowitz_layer(true_returns_solver, risk_factor)
