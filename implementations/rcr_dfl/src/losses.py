...
def markowitz_cost(
    ...
    f(w, mu_true)
    = lambda * w^T Sigma_eff w - mu_true^T w
    = ||Lw||_2^2 - mu_true^T w
    """
    ...
    if risk_factor.shape[-2:] != (
        ...
        raise ValueError(
            ...
            f"weights={tuple(weights.shape)}, "
            f"risk_factor={tuple(risk_factor.shape)}"
        )
...
def rcr_regret(
    ...
    """
    Residual Collective Risk regret.

    risk_factor가 Sigma_eff에서 계산되므로 이 regret은
    수익률, 기존 covariance risk, residual collective risk를
    모두 포함한 의사결정 손실이다.
    """
...
def compute_rcr_losses(
    ...
    """한 batch의 MSE, RCR regret, combined loss를 함께 계산한다."""
