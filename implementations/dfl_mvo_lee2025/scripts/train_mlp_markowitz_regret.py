...
def markowitz_cost(
    ...
    """
    f(w, mu*) = lambda * w.T Sigma w - mu*.T w

    risk_factor.T @ risk_factor = lambda * Sigma 이므로
    lambda * w.T Sigma w = ||risk_factor @ w||^2 이다.
    """
...
def compute_regret_loss(
    ...
    # 실제 수익률을 알고 있었다고 가정한 oracle 포트폴리오
    ...
    return regret_loss, predicted_returns
...
def run_epoch(
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_training):
            regret_loss, predicted_returns = compute_regret_loss(
                model=model,
                features=features,
                targets=targets,
                covariance=covariance,
        ...
        total_regret += regret_loss.item() * batch_size
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"Validation Regret: {validation_regret:.8f} | "
    ...
    print(f"Best Validation Regret: {best_validation_regret:.8f}")
    print(f"Test Regret: {test_regret:.8f}")
